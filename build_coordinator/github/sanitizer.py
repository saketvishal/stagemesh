...
- leak private repository information (especially caventra-private) into public/product issues or comments
"""
...
PRIVATE_LEAK_PATTERNS = [
    re.compile(r"caventra-private", re.IGNORECASE),
    re.compile(r"patent-sensitive", re.IGNORECASE),
    re.compile(r"PATENT_COPY", re.IGNORECASE),
    re.compile(r"CAVENTRA_CAPABILITY_MAP", re.IGNORECASE),
    re.compile(r"CAVENTRA_PRIVATE_ROADMAP", re.IGNORECASE),
    re.compile(r"CAVENTRA_PRODUCT_PRINCIPLES", re.IGNORECASE),
]
...
def sanitize_issue_to_spec(
    ...
    *,
    ...
    # If the target repository is NOT caventra-private, strictly prohibit referencing caventra-private
    if validated_repo != "caventra-private":
        prohibited_scope = ("caventra-private/**", "docs/patent/**")
...
def redact_private_info(text: str) -> str:
    """Prevent information from caventra-private from leaking into public/product text."""
    if not text:
        return ""
    result = text
