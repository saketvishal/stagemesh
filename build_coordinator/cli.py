...
def _controller_source() -> dict[str, str]:
    """Report which `build_coordinator` checkout this running
    process actually loaded its code from. If a stale checkout on cwd (e.g.
    another worktree's own `build_coordinator/`) ever shadowed the
    intended controller, this is what would reveal it: `cli_module_file`
    would point outside the configured control repo root."""
    import build_coordinator as _package
...
def _seed(args: argparse.Namespace, session) -> None:
    for spec in LEGAL_INTELLIGENCE_EVALUATION_TASKS:
        upsert_task(session, spec)
    _print({"seeded": len(LEGAL_INTELLIGENCE_EVALUATION_TASKS)})
...
def _runner(args: argparse.Namespace, session) -> None:
    ...
    if getattr(args, "github", False):
        ...
        controller = GitHubAutonomousController(SessionLocal)
        if args.once:
            res = controller.run_once()
            _print(
                {
                    "issues_ingested": res.issues_ingested,
                    "gates_approved": res.gates_approved,
                    "gates_published": res.gates_published,
                    "prs_created": res.prs_created,
                    "statuses_synced": res.statuses_synced,
                }
            )
            return
